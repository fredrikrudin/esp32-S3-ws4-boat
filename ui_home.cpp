/* Home tab: the start page, in the style of the Victron GX Boat Page.
   ...
   summary line for temperatures and depth at the bottom. */
#include "app.h"
#include "n2k_data.h"
...
static lv_obj_t *lbl_inside, *lbl_outside, *lbl_depth;
...
void build_home_tab() {
  ...
  /* bottom strip: temperatures and depth */
  ...
  lbl_depth = lv_label_create(strip);
...
void home_timer_cb(lv_timer_t *t) {
  ...
  /* ---- battery: Victron battery monitor ---- */
  ...
  /* depth from NMEA 2000 (plus the sounder's offset, as on the Nav tab) */
  double d, off;
  if (n2kGet(Q_DEPTH, 0, 0, d)) {
    if (n2kGet(Q_DEPTH_OFFSET, 0, 0, off, 30000)) d += off;
    snprintf(b, sizeof(b), LV_SYMBOL_DOWN " %.1f m", d);
  } else {
    strcpy(b, LV_SYMBOL_DOWN " --");
  }
  set_label(lbl_depth, b);
}
